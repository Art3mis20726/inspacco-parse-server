...
async function execute(
  ...
  switch (req.params.type) {
    case 'AVAIL_SERVICE':
      return availService(req);
    case 'PROVIDE_SERVICE':
      return provideService(req);
    case 'ONBOARDING':
...
async function availService(req: Parse.Cloud.FunctionRequest) {
  ...
  await sendEmail(
    sendEmailTo,
    '[Inspacco Mobile] Avail service request',
    renderFile('./src/emailTemplates/availServiceRequest.pug', {
      contactNumber: req.user.get('mobileNumber'),
      name: `${req.user.get('firstName')} ${req.user.get('lastName')}`,
